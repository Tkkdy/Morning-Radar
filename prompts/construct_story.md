...
claim_type、requested_scope、evidence_scope 和 claim_scope。事实主体由调用方根据 Candidate 与
所选 Evidence 确定性推导，模型不得提供或选择 claim_subject。requested_scope 使用
...
全球 GA 或所有用户可用。独立媒体报道不等于独立验证；performance/first claim 需要 Evidence
明确携带 independently-verified scope。Discovery-only Evidence 不能进入 facts。
